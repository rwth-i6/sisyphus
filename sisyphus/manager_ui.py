...
class UiLoggingHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        self.add_to_logger_box(
            urwid.Text([record.asctime, " ", record.levelname, " ", (color, record.getMessage())], wrap="clip")
        )
...
help_text = (
    """Welcome to the sisyphus manager interface and its currently very short help page
...
class SisyphusDisplay:
    ...
    def show_job(self, job):
        ...
        items.append(
            add_button(
                "Action: remove job and descendants", self.run_commands, [(tk.remove_job_and_descendants, [job], {})]
            )
        )

        items.append(add_button("Setup job directory", self.run_commands, [(job._sis_setup_directory, [], {})]))
    ...
    def setup_view(self):
        ...
        self.question_view = urwid.Overlay(
            self.question_text, self.main_view, "center", ("relative", 80), "middle", None
        )
    ...
    def update_menu(self):
        ...
        add_button(
            "cleanup work directory (keep value=0)",
            self.run_commands,
            [(tk.cleaner, [], {"clean_work_dir": True, "mode": "remove", "keep_value": 0})],
        )
        add_button(
            "cleanup work directory (keep value=40)",
            self.run_commands,
            [(tk.cleaner, [], {"clean_work_dir": True, "mode": "remove", "keep_value": 40})],
        )
        add_button(
            "cleanup work directory (keep value=60)",
            self.run_commands,
            [(tk.cleaner, [], {"clean_work_dir": True, "mode": "remove", "keep_value": 40})],
        )
        add_button("cleanup job directories", self.run_commands, [(tk.cleaner, [], {"clean_job_dir": True})])
    ...
    def update_job_view(self, jobs=None, overwrite_update_stop=False, list_all_jobs=False):
        ...
        for state, job, info in self.current_jobs:
            ...
            if state in [gs.STATE_INPUT_MISSING, gs.STATE_RETRY_ERROR, gs.STATE_ERROR]:
                attri = "error"
