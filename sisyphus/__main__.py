...
def main():
    ...
    parser = argparse.ArgumentParser(description=gs.SISYPHUS_LOGO, formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="config file for setup, defining which jobs to run",
    )
    ...
    parser_manager = subparsers.add_parser(
        "manager", aliases=["m"], conflict_handler="resolve", help="Load config files and start manager loop"
    )
    parser_manager.set_defaults(func=manager)
    parser_manager.add_argument("-r", dest="run", default=False, action="store_true", help="Run the given task")
    ...
    parser_manager.add_argument(
        "-i", "--interactive", dest="interactive", default=False, action="store_true", help="Ask before submitting jobs"
    )
    ...
    parser_console.add_argument("--load", dest="load", default=[], action="append", help="load graph and start console")
    parser_console.add_argument(
        "--skip_config",
        dest="not_load_config",
        default=False,
        action="store_true",
        help="do not load config files before starting the console",
    )
    parser_console.add_argument(
        "-c", dest="commands", default=[], action="append", help="Run commands after loading console"
    )
    parser_console.add_argument(
        "--script",
        "-s",
    ...
    parser_worker.add_argument("jobdir", metavar="JOBDIR", type=str, help="Job directory of the executed function")
    parser_worker.add_argument("task_name", metavar="TASK_NAME", type=str, help="Task name")
    ...
    parser_worker.add_argument(
        "--redirect_output", default=False, action="store_true", help="Redirect stdout and stderr to logfile"
    )
    ...
    if args.func == manager and args.ui:
        ...
        ui.setup()
        logging.basicConfig(
            format="[%(asctime)s] %(levelname)s: %(message)s", level=args.log_level, handlers=[ui.get_log_handler()]
        )
    else:
    ...
    try:
        ...
        non_daemon_threads = {
            thread for thread in threading.enumerate() if not thread.daemon and thread is not threading.main_thread()
        }
